...
public interface checkPhoneNumber {
    ...
    static final int MIN_SIZE_PHONE_NUM = 4;
    static final int MAX_SIZE_PHONE_NUM = 16;
    ...
    default boolean checkPhoneNumber(String s) {
        ...
        try {
            if (s.length() > MIN_SIZE_PHONE_NUM && s.length() < MAX_SIZE_PHONE_NUM && Long.parseLong(s) > 0) {
                ...
                System.out.println("Номер слишком длинный. Допустимо от 5 до 15 символов включительно");
            } else {
                System.out.println("Номер слишком короткий. Допустимо от 5 до 15 символов включительно");
            }
